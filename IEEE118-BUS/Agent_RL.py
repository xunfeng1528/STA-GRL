...
class BCERPG:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim,  actor_lr, critic_lr, device, tau = 0.005, gamma = 0.98,sigma = 0.01):
        self.actor = STConv(118, 1, 32, 1, 3, 10).to(device)
    ...
    def adjust_power_output(self,action, pmin, pmax):
        adjusted_action = torch.where(action < pmin / 2, torch.zeros_like(action), action)
        adjusted_action = torch.where(
            (adjusted_action >= pmin / 2) & (adjusted_action < pmin), pmin, adjusted_action)
        adjusted_action = torch.where(adjusted_action > pmax, pmax, adjusted_action)
    ...
    def update(self, transition_dict, net, batch_size, epochs, state_dict, pd_factor, pmin_factor, pmax_factor, cost_factor):
        if net == 'critic':
            ...
            #先计算了梯度成本
            action_new = F.relu(self.actor(states).reshape(-1, 6, 54))#对不同的状态生成了不同的调度计划，在这里生成批调度规划
            adjusted_action_new = self.adjust_power_output(action_new, p_min, p_max)
            # 计算成本函数
            re_1_intermediate = a * adjusted_action_new ** 2 + b * adjusted_action_new + c
            re_1_summed_6 = re_1_intermediate.sum(dim=2)
            re_new = re_1_summed_6.sum(dim=1)
            # 计算供需平衡惩罚
            prod_demand_re = torch.abs(adjusted_action_new.sum(dim=2) - demand)
            prod_demand_penalty = prod_demand_re.sum(dim=1)
            # 计算最小约束违反惩罚
            min_violation_re = torch.where(
                adjusted_action_new < 0,
                -adjusted_action_new,
                torch.where(
                    (adjusted_action_new > 0) & (adjusted_action_new < p_min),
                    p_min - adjusted_action_new,
                    torch.zeros_like(adjusted_action_new)
                )
            )
            min_violation_penalty = min_violation_re.sum(dim=2).sum(dim=1)
            # 计算最大约束违反惩罚
            max_violation_re = F.relu(adjusted_action_new - p_max)
            max_violation_penalty = max_violation_re.sum(dim=2).sum(dim=1)
            # 计算总惩罚
            total_penalty = pd_factor*prod_demand_penalty + pmin_factor * min_violation_penalty + pmax_factor * max_violation_penalty
            re_all = total_penalty.reshape(-1, 1)
...
default_supply_df = pd.read_csv('data/processed_supply_file118.csv')
...
pd.Series(actor_loss_list).rolling(50).mean().plot(label='actor_loss_change_rolling',color='blue')
plt.xlabel('Episodes')
...
pd.Series(return_list).rolling(50).mean().plot(label='cost_change_rolling',color='blue')
plt.xlabel('Episodes')
